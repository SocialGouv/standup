...
# clean the repo, be carefull when running this command !
# rm -rf ./*

mkdir -p autodevops-helm
# npx degit https://github.com/SocialGouv/actions/autodevops-helm autodevops-helm
npx degit https://github.com/SocialGouv/standup/.github/actions/deploy#helm-template-manifests autodevops-helm --force
yarn --cwd autodevops-helm install
yarn --cwd autodevops-helm run -s values > values.env.yaml

mkdir -p project/.socialgouv
if [ -d "$PROJECT_PATH/.socialgouv" ]; then
  cp -r $PROJECT_PATH/.socialgouv/. project/.socialgouv
else
  npx degit https://github.com/SocialGouv/$PROJECT_REPO_NAME/.socialgouv#helm-template-manifests project/.socialgouv --force
fi
